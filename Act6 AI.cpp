...
int main() {
 ...
 //- PREV CODE USED FOR CHECKING AND INTERACTION; DOES NOT CHECK IF COMPUTER WILL REPEAT PREV GUESSES
    ...
    */
    ...
    //COMPUTER CHECKS TO NOT REPEAT PREV GUESSES
for(int i = 0; PlayerCurrGuess != AiCode && AiCurrGuess != PlayerCode; i++){
    cout << "-----Turn : " << i + 1 << " -----" << endl;
    ...
    do {
        AiCurrGuess = Generator();
    } while (isAlreadyGuessed(AiGuessHistory, AiCurrGuess) || AiCurrGuess == PlayerCurrGuess);
